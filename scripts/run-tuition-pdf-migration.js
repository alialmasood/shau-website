/**
 * تشغيل هجرة tuition_pdf يدوياً (بديل عن npx prisma migrate deploy عند مشاكل الشبكة).
 * الاستخدام: node scripts/run-tuition-pdf-migration.js
 */
...
const migrationPath = path.join(
  ...
  "prisma",
  "migrations",
  "20260125300000_add_tuition_pdf",
  "migration.sql"
);

async function run() {
  console.log("تشغيل هجرة tuition_pdf...\n");
  ...
  const check = await pool.query(`SELECT 1 FROM tuition_pdf LIMIT 1`).catch(() => null);
  console.log("\nتم تطبيق هجرة tuition_pdf بنجاح.");
  if (check) console.log("الجدول tuition_pdf موجود ويستجيب.");
}
