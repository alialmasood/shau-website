/**
 * ضبط صلاحيات مستخدم results@shau.edu.iq
 * - تحديث الدور إلى EXAM_COMMITTEE
 * - منح صلاحيات كاملة لصفحة results
 */
...
const TARGET_EMAIL = "results@shau.edu.iq";
const PAGE_CODE = "results";
...
async function run() {
  ...
  try {
    ...
    // 2) تحديث الدور إلى EXAM_COMMITTEE
    await client.query(
      `UPDATE admin_users SET role = 'EXAM_COMMITTEE' WHERE id = $1`,
      [userId]
    );
    ...
    if (pageRes.rows.length === 0) {
      throw new Error(`صفحة النتائج غير موجودة في admin_pages: ${PAGE_CODE}`);
    }
    const pageId = pageRes.rows[0].id;

    // 4) منح صلاحيات كاملة لصفحة النتائج
    await client.query(
    ...
    console.log(`- الدور: EXAM_COMMITTEE`);
    console.log(`- الصلاحيات: كاملة على صفحة results`);
  } catch (error) {
