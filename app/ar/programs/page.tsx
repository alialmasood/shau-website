...
export default async function ArProgramsPage() {
  ...
  return (
    <div className="w-full min-h-screen bg-white">
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-10">
          <h1 className="text-3xl md:text-4xl font-bold text-neutral-900">{(t as { programs?: { title?: string } }).programs?.title ?? "برامج الكلية"}</h1>
          <div className="w-20 h-1 bg-[#31BD9C] mt-3 rounded-full" />
        </div>

        {programs.length === 0 ? (
          <p className="text-neutral-600 py-12">{tp.noPrograms ?? "لا توجد برامج متاحة."}</p>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-sm text-neutral-600 mt-1 line-clamp-2">{(p.briefAr || "").slice(0, 100)}...</p>
                    <span className="inline-flex items-center gap-1 mt-3 text-[#31BD9C] font-semibold text-sm">
                      {tp.viewDetails ?? "عرض التفاصيل"}
        ...
        <div className="mt-10">
          <Link href="/ar" className="text-[#31BD9C] font-semibold hover:underline">← الرئيسية</Link>
        </div>
      </div>
    </div>
  );
