import { getActiveSocialLinks } from "@/lib/socialMediaRepo";
...
type SocialMediaButtonsProps = {
  /** صنف إضافي للحاوية */
  className?: string;
  /** حجم الأزرار: default | sm */
  size?: "default" | "sm";
  /** اتجاه الأزرار: row للفوتر، column للهيرو (عمودي) */
  direction?: "row" | "column";
};

/**
 * يعرض أزرار السوشيال ميديا الدائرية (خلفية #31BD9C وأيقونات بيضاء)
 * من البيانات المخزنة في قاعدة البيانات. يمكن استدعاؤه في أي سكشن (Footer، Hero، Contact...).
 */
export default async function SocialMediaButtons({ className = "", size = "default", direction = "row" }: SocialMediaButtonsProps) {
  ...
  const containerCls = direction === "column"
    ...
    : `flex flex-wrap justify-center gap-1.5 sm:gap-2 ${className}`;

  return (
    <div className={containerCls}>
      {links.map((link) => {
        const href = link.url ?? "#";
        const icon = ICONS[link.platform] ?? null;
