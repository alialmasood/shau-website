...
const getCachedNews = cache((id: string, locale: "ar" | "en") =>
...
export type NewsDetailsViewProps = {
  params: Promise<{ id: string }>;
  basePath: "/ar" | "/en";
};

export default async function NewsDetailsView({
  params,
  basePath,
}: NewsDetailsViewProps) {
  const { id: rawId } = await params;
  const locale = basePath === "/en" ? "en" : "ar";
  const t = getTranslations(locale);
  const news = await getCachedNews(rawId, locale);
  if (!news) notFound();
  ...
  const dateFmt = locale === "ar" ? "ar-IQ" : "en-GB";
  const dateLabel = news.publishedAt
    ? new Intl.DateTimeFormat(dateFmt, {
        year: "numeric",
        month: "long",
        day: "numeric",
      ...
      <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-7 sm:py-10">
        <nav className="text-xs sm:text-sm text-neutral-500 mb-5">
          <ol className="flex flex-wrap items-center gap-2">
            <li>
              <Link href={basePath} className="hover:text-[#31BD9C] transition-colors">
                {t.common.home}
              </Link>
            </li>
            ...
            <div className="mb-6">
              <div className="relative w-full rounded-2xl overflow-hidden border border-neutral-100">
                <div className="relative w-full aspect-[16/9]">
                  <Image
                    src={secondarySrc}
                    alt={t.newsDetails.secondaryImageAlt.replace("{title}", news.title)}
                    fill
                    className="object-cover"
                    unoptimized
                  />
