...
export default function StudentAccountsBatchCard({
  ...
  async function handleDelete() {
    ...
    startTransition(async () => {
      try {
        const result = await deleteStudentAccountsBatchAction(batch.id);
        if (result.success) {
          router.refresh();
          // If this batch was selected, redirect to student-accounts page without batch filter
          if (isSelected) {
            router.push("/admin/student-accounts");
          }
        } else {
  ...
  return (
    ...
    >
      <Link
        href={`/admin/student-accounts?batchId=${batch.id}`}
        prefetch={false}
        className="block"
      >
        ...
        <div className="flex items-center gap-3 mt-2 text-xs">
          ...
          {batch.updatedCount > 0 && (
            <span className="text-blue-600">🔄 {batch.updatedCount} محدث</span>
          )}
